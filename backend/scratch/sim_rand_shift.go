	...
	for _, maxNibbleMismatches := range maxNibbles {
		...
		for offset := 0; offset <= len(ref)-len(qry); offset++ {
			...
			for i := 0; i < len(qry); i++ {
				// WITH SHIFT
				if offset+i < len(ref) {
					bestD := matching.HammingHex(ref[offset+i], qry[i])
					if offset+i > 0 {
						d := matching.HammingHex(ref[offset+i-1], qry[i])
						if d < bestD {
							bestD = d
						}
					}
					if offset+i+1 < len(ref) {
						d := matching.HammingHex(ref[offset+i+1], qry[i])
						if d < bestD {
							bestD = d
						}
					}
					if bestD <= maxNibbleMismatches {
						matches++
					}
				}
			}
		...
		fmt.Printf("Random Noise (WITH Shift), Max Mismatches = %d, Peak = %.2f%%\n", maxNibbleMismatches, bestConf*100)
	}
}
